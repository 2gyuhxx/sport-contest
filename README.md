...
### [이름 1] - 프론트엔드 개발
> 이번 프로젝트를 통해 지도 시각화 기술을 처음 접하게 되었습니다. TopoJSON 데이터를 React 컴포넌트로 렌더링하고, 사용자 인터랙션을 처리하는 과정이 가장 어려웠습니다. 또한 성능 최적화를 위해 코드 분할과 메모이제이션을 적용하면서 React의 렌더링 최적화 원리를 깊이 이해할 수 있었습니다. Vite를 사용하면서 개발 속도가 크게 향상된 것을 체감했습니다.

### [이름 2] - 백엔드 개발
> Express와 MySQL을 활용한 RESTful API 개발을 경험했습니다. JWT 인증 구현과 OAuth 연동 과정에서 보안의 중요성을 배웠습니다. 특히 스팸 필터링을 위해 Node.js에서 Python 프로세스를 실행하는 방식으로 ML 모델을 통합하는 것이 흥미로웠습니다. 프로덕션 배포 시 PM2와 Nginx 설정을 통해 안정적인 서비스 운영의 기반을 마련했습니다.

### [이름 3] - ML 모델 개발
> 코사인 유사도 알고리즘을 실제 프로젝트에 적용해보면서 협업 필터링의 원리를 이해할 수 있었습니다. 사용자 행동 데이터를 행렬로 변환하고 유사도를 계산하는 과정이 수학적으로 복잡했지만, 결과적으로 사용자 만족도를 높일 수 있었다는 점이 뿌듯합니다. BERT 모델을 활용한 스팸 필터링에서는 텍스트 분류의 기본 개념을 배웠고, 실제로 효과적으로 부정 행사를 차단할 수 있었습니다.

### [이름 4] - 풀스택 개발
> 프론트엔드와 백엔드를 모두 개발하면서 전체 시스템 아키텍처를 이해할 수 있었습니다. 코드 분할과 API 최적화를 통해 성능 개선에 기여했고, 배포 자동화 스크립트를 작성하여 운영 효율성을 높였습니다. 팀원들과의 협업 과정에서 Git 브랜치 전략과 코드 리뷰의 중요성을 배웠습니다.
